...
describe("Ticket 5: Admin Reschedule Dialog & Duty Schedule Leave Management with Collision Alerts", () => {
	...
	it("applies soft block for doctor leaves and flags collision warning (ADR-0002)", async () => {
		...
		// Spec contract line 74: schedules.addBlock
		const blockRes = await caller.schedules.addBlock({
			dentistId: "dentist_may",
			date: "2026-09-25",
			startTime: "13:00",
			endTime: "16:00",
		...
		expect(blockRes.id).toBeDefined();
		...
		).rejects.toThrow("ทันตแพทย์ไม่มีตารางปฏิบัติงานในวันหรือช่วงเวลาดังกล่าว");
